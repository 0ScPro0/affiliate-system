...
if [ "$DATABASE_POSTGRES_HOST" = "localhost" ]; then
    echo "Warning: DATABASE_POSTGRES_HOST is localhost, overriding to affiliate-system-postgres for Docker"
    DB_HOST="affiliate-system-postgres"
else
...
docker compose run --rm affiliate-system-migrate -path /migrations -database "$MIGRATE_URL" "$1"
